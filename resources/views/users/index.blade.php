            ...
            <div class="col-xs-12">
                <div class="box">
                    <div class="box-header">
                        ...
                        <a href="{{ action('UserController@create') }}" class="btn btn-success"><span
                        ...
                        <table id="users-table" class="table table-bordered table-hover">
                            ...
                            @foreach($users as $index => $user)
                                <tr>
                                    ...
                                    <td>{{ $user->name }}</td>
                                    <td>{{ $user->email }}</td>
                                    @if ($user->role === config('user.role.CA'))
                                        <td>Change Administrator</td>
                                    @else
                                    ...
                                    <td>
                                        ...
                                        @if ($user->email !== Auth::user()->email)
                                            ...
                                            {!! Form::close() !!}
                                        @endif
                                        @if (!$user->is_approved && Auth::user()->role == config('user.role.CA'))
                                        {!! Form::open(['action' => ['UserController@approveUser', $user->id],
                                                'method' => 'POST',
                                                'class' => 'inline',
                                                ]) !!}

                                            {{ Form::button('<i class="glyphicon glyphicon-ok"></i>', ['type' => 'submit', 'class' => 'btn btn-primary', 'onClick' => 'return window.confirm("Are you sure?")']) }}
                                            {!! Form::close() !!}
                                        @endif
                                    </td>
                                </tr>
