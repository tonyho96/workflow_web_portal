    ...
    <div class="login-box">
        ...
        <div class="login-box-body">
            ...
            <form action="{{ url(config('adminlte.password_email_url', 'password/email')) }}" method="post">
                ...
                @if (session('status'))
                    <a href="{{ route(config('adminlte.login_url')) }}">
                        <button style="margin-bottom: 15px" type="button" class="btn btn-primary btn-block btn-flat">
                            {{ config('adminlte.login_text') }}</button>
                    </a>
                @endif
